...
ip_addr = vcfg.get_value("ip_addr").encode()
dev_id = c_uint32(vcfg.get_value("dev_id"))
media_file = vcfg.get_value("media_file").encode()
afbc_enable = c_int32(vcfg.get_value("afbc_enable"))
hist_enable = c_int32(vcfg.get_value("hist_enable"))
hist_threshold = c_int32(vcfg.get_value("hist_threshold"))
process_pool_max = c_int32(vcfg.get_value("process_pool_max"))
...
class Test(TestCase):
    ...
    def test_multi_decode(self):
        decode_info_list = [{"ip": "TCP:192.168.1.10:9000", "id": 0},
                            {"ip": "TCP:192.168.1.10:9000", "id": 1},
                            {"ip": "TCP:192.168.1.10:9000", "id": 2},
                            {"ip": "TCP:192.168.1.10:9000", "id": 3},
                            ]
        process_pool = multiprocessing.Pool(process_pool_max)
        for info in decode_info_list:
            process_pool.apply_async(func=self.decode, args=(info.get("ip").encode(), c_int32(info.get("id")))   ##非阻塞

            process_pool.close()
            rocess_pool.join()
            print("multi decode process end")
        return True
...
if __name__ == '__main__':
    pass
    # suite = TestSuite()
    # suite.addTest(Test('test_multi_decode'))
